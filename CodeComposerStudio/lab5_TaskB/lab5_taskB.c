...
Queue_Handle uart_queue;
MsgObj msg_mem[COORDBUFFERSIZE];
...
void enterFxn()
{
    ...
    if(Semaphore_pend(semHandle, BIOS_NO_WAIT)) {
        recv_msg = Queue_get(uart_queue);
        System_printf("enterFxn: id = %d and val = '%d'.\n",
...
void backspaceFxn()
{
    ...
    if(Semaphore_pend(semHandle, BIOS_NO_WAIT)) {
        recv_msg = Queue_get(uart_queue);
        System_printf("backspaceFxn: id = %d and val = '%d'.\n",
                      recv_msg->id, recv_msg->data);
    }
...
int main(void)
{
    ...
    // create uart queue
    uart_queue = Queue_create(NULL, NULL);

    msg1.id = 11;
    msg1.data = 20;

    Queue_put(uart_queue, &msg1.elem);
    msg2 = Queue_get(uart_queue);
